...
def silhouette_plots(adata, pathway_names, pathway_genes, metric='cosine', norm = False, ax=None,key="leiden"):
    '''This function gives silhouette scores and boxplots for different numbers 
    of clusters on our pathways.
...
def weight_dist(ge_df, weight_df):
    ...
    Returns:
    A 1-D condensed distance matrix for distances between all pairs of cluster labels.'''
...
def weighted_silhouette_plot(adata, pathway_names, 
                             pathway_genes, weight_df, key = "leiden", metric='euclidean', method = 'complete',
                             norm = True, ax=None):
    ...
    for i in range_n_clusters:
        ...
        if np.array_equal(linkage,np.array([1]*len(adata.obs[key].unique()))):
            range_n_clusters.remove(2)
        else:
            sil_score = silhouette_score(X, linkage, metric="precomputed", method='complete')
            sil_score_list.append([sil_score])
            sil_score = []
    ax.plot(range_n_clusters, sil_score_list, '-o', markersize=0.75);
...
def weighted_heatmap(adata, pathway_genes, pathway_pvals, num_clust, name, key = "leiden",norm = True,
                    ...
                    metric = "cosine"):
    '''We group the leiden clusters based on similarity of expression of 
    specific genes in a pathway. We weight this similarity based on differentially expressed genes between all leiden clusters.
    
    Arguments :
    
    adata : the AnnData gene expression matrix
    pathway_genes : a list of the genes in the pathway
    pathway_pvals: a lower traingular dataframe that gives weights for differentially expressed genes for all combinations of labels.
    num_clust : the optimal number of clusters based on silhouette score on
    a given metric (automatically cosine) distance
    name : the name with which we want to label the clusters of this pathway
    leg_axes : we can change the coordinates of the legend
